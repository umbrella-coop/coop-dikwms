...
- Layout (SPEC-020 context-first, Bit standard, no SSR — client-only apps):
  ...
  - `dikwms/ui/<context>/<name>` — visual components per bounded context (`data-graph`, `data-graph-antv-g6` — G6 rendering engine, `layout` — app-shell layout system, `organization` — org console, `workspace` — workspace console, `project` — project views, reserved: `data-graph-governance`, `iam`, `data-schema-registry`)
