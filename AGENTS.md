...
> The product is a **Data, Information, Knowledge and Wisdom (DIKW) Management System** — acronym **dikwms** — based on Jennifer Rowley's DIKW hierarchy paper as the base taxonomy: *"The wisdom hierarchy: representations of the DIKW hierarchy"*, Journal of Information Science, 33(2), 2007 [Source: External — Rowley, 2007; critics acknowledged, adopted as base literature for architecture/design].
...
4. **Integration tests use `TerminusDBServer::test_instance()`** (real per-process server, official `terminusdb/terminusdb` @ stable `v12.0.7` — user directive 2026-08-11; override `TERMINUSDB_VERSION`) — never mocks for persistence. Exception: **`git-importer` tests use the compose-hosted server** (`docker compose up -d terminusdb`; env `TERMINUSDB_URL`/`TERMINUSDB_ADMIN_PASS`) — no embedded source build for that crate (user directive 2026-08-11). Heavy concurrent tests against the shared server may hit transaction contention: serialize with a global mutex (see `api/tests/spec_013_api.rs`) or `RUST_TEST_THREADS=1`.
