...
6. **check-layout.mjs (v2, SPEC-020 AC-8):** enforces `{app, hook, type, ui}` + context whitelist (`data-graph`, `data-graph-antv-g6`, `data-graph-governance`, `iam`, `organization`, `data-schema-registry`) + `index.ts` presence.
